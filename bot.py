...
# ========= CONFIGURATION =========
# The bot token is loaded from an "Environment Variable" on your hosting platform (Render).
...
# ========= LOGGING SETUP =========
# Configures logging to show informational messages, crucial for debugging on the server.
logging.basicConfig(
...
# This simple web server keeps the bot running 24/7 on free hosting services.
...
# Decorator to check if commands are enabled and to prevent crashes from non-text messages.
...
`/setremain <number>` - Set daily like limit.
`/broadcast <message>` - Send message to all.
`/on` / `/off` - Enable or disable commands.
"""
    await update.message.reply_text(admin_text, parse_mode='Markdown')

# ========= LIKE COMMAND (Main Feature) =========
@check_command_enabled
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    today = get_today()
    user_info = user_data.get(user_id, {})
    status = "UNLIMITED (VIP)" if user_id in vip_users else ("✅ Used" if user_info.get("date") == today else "❌ Not Used")
...
# This 'on' command intentionally does NOT use the decorator, so it can be run when commands are off.
...
def main() -> None:
    ...
    if loop.is_running():
        ...
        # This is for local testing where a loop might not be running yet
        loop.run_until_complete(reset_group_usage_task())
